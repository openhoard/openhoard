...
| OAuth access token (`ohat.`, `/mcp`) | any stop, `unlinkIdentity()`, the client revoked (admin API), `revokeGrant()`, the client's RFC 7009 revocation of its refresh token | the grant is revoked, and with it every token it holds                                                                   | next request; a request already running doesn't deliver its answer (`grantIsLive()` again before it leaves) |
| OAuth access token                   | the client's RFC 7009 revocation of that access token                                                                                | that token alone is deleted; the grant and its refresh token stay                                                        | next request; a request already running doesn't deliver its answer (`grantIsLive()` checks the token too)   |
| OAuth refresh token (`ohrt.`)        | as the first access-token row (its grant revoked)                                                                                    | refused (`invalid_grant`)                                                                                                | next token request                                                                                          |
